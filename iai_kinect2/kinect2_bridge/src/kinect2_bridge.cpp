...
private:
  ...
  enum Image
  {
    ...
    DEPTH_SD, //no remap or registerDepth
    DEPTH_SD_RECT,
    DEPTH_HD,
    DEPTH_QHD,
...
private:
  ...
  void initTopics(const int32_t queueSize, const std::string &base_name)
  {
    ...
    topics[DEPTH_SD] = K2_TOPIC_SD K2_TOPIC_IMAGE_DEPTH;// /sd/image_depth
    ...
    topics[COLOR_HD] = K2_TOPIC_HD K2_TOPIC_IMAGE_COLOR;// /hd/image_color
...
//ir1**********************************get ir and depth using libfreenect2, libfreenect2::Frame *depthFrame
...
//images[] have been processed to fill a certain type, for example cv:Mat images[DEPTH_SD] = depth
  ...
  void receiveColor()
  {
    ...
    //color1****************************get color using libfreenect2, libfreenect2::Frame *colorFrame
  ...
  void processIrDepth(const cv::Mat &depth, std::vector<cv::Mat> &images, const std::vector<Status> &status)
  {
    ...
    if(status[DEPTH_SD])
    {
      //ir3***************************from depth convertTo images[DEPTH_SD],and flip function.
      depth.convertTo(images[DEPTH_SD], CV_16U, 1);
      cv::flip(images[DEPTH_SD], images[DEPTH_SD], 1);
    }*/
  ...
  void publishImages(const std::vector<cv::Mat> &images, const std_msgs::Header &header, const std::vector<Status> &status, const size_t frame, size_t &pubFrame, const size_t begin, const size_t end)
  {
    ...
    {
      _header.frame_id = baseNameTF + K2_TF_RGB_OPT_FRAME;  //will be overrided in the future for loop
    ...
    for(size_t i = begin; i < end; ++i)
    {
      ...
      case RAW:
        ...
        //4**********************memcpy cv::Mat images[i] data to sensor_msgs::Image* imageMsgs[i]
        break;
      case COMPRESSED:
    ...
    while(frame != pubFrame) //wait until the last frame have been published
    {
